...
class TestIndex(TestCase):
    ...
    def test_admin_login_redirect(self):
        ...
        self.assertRedirects(response, "/login/?next=%2Fadmin%2F", target_status_code=302)
...
class TestMenu(TestCase):
    ...
    def test_menu(self):
        menu_registry.register("test2", "Test Menu Item 2", lambda request: "/test_menu_link2", order=2)
        menu_registry.register("test1", "Test Menu Item 1", lambda request: "/test_menu_link1", order=1)
        ...
        self.assertLess(response.index("Test Menu Item 1"), response.index("Test Menu Item 2"))
    ...
    def test_side_menus_registry(self):
        ...
        try:
            ...
            menu_registry.register("test", "Test Menu Item", lambda request: "/test_menu_link", order=2)
            admin_menu_registry.register("test_admin", "Test Admin Item", lambda request: "/spam", order=100)
            ...
            self.assertLess(response.index("Test Admin Item"), response.index("Test Menu Item"))
        finally:
...
if 'oioioi.default_settings' in sys.modules:
    ...
    sys.exit(0)"""
        ret = subprocess.call([sys.executable, "-c", subprocess_code], env=subprocess_env)
...
class TestExecute(TestCase):
    ...
    def test_line_splitting(self):
        self.assertListEqual([b"foo", b"lol"], execute('echo "foo\nlol"', split_lines=True))
...
class TestMisc(TestCase):
    ...
    def test_uploaded_file_name(self):
        tmp_file = TemporaryUploadedFile("whatever", "application/octet-stream", 0, "utf-8")
...
class TestArchive(TestCase):
    ...
    def test_size_calc(self):
        for good_file, expected_size in zip(self.good_files, (8, 8), strict=False):
            filename = os.path.join(self.base_dir, good_file)
...
class TestBaseViews(TestCase):
    ...
    def test_edit_profile_view(self):
        ...
        self.assertIn("registration/registration_form.html", [t.name for t in response.templates])
    ...
    def test_unicode_wrong_first_name(self):
        self.data["first_name"] = "wrong_unicode_\U0001f923"
    ...
    def test_unicode_wrong_last_name(self):
        ...
        self.data["last_name"] = "wrong_unicode_\U0001f923"
        response = self.client.post(self.url_edit_profile, self.data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.user.refresh_from_db()
        # Check that the name is not changed
    ...
    def test_names_with_valid_spaces(self):
        self.data["first_name"] = ("Jan Maria",)
        self.data["last_name"] = ("Le Guien",)
        response = self.client.post(self.url_edit_profile, self.data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.user.refresh_from_db()
        self.assertEqual(self.user.first_name, "Jan Maria")
        self.assertEqual(self.user.last_name, "Le Guien")
    ...
    def test_profile_dynamic_fields(self):
        ...
        try:
            PreferencesFactory.add_field("dog", CharField, lambda n, u: "Andrzej", label="Doggy")
...
class TestNotifications(TestCase):
    ...
    def test_notification_registration(self):
        ...
        def notification_function(arguments):
            ...
            message_arguments = {}
            user = User.objects.get(username="test_user")
            NotificationHandler.send_notification(user, "test_notification", message, message_arguments)
            flags["got_notification"] = True

        NotificationHandler.register_notification("test_notification", notification_function)
...
class TestLoginChange(TestCase):
    ...
    def test_message(self):
        for value in self.invalid_logins:
        ...
        for value in self.valid_logins:
            self.user.username = value
            self.user.save()
    ...
    def test_can_change_login_from_invalid(self):
        for value in self.invalid_logins:
            ...
            self.assertEqual(self.user.username, value)
    ...
    def test_failed_login_change(self):
        ...
        for login in self.invalid_logins:
            self.client.post(url_edit_profile, {"username": login, "terms_accepted": True}, follow=True)
...
class TestAccountDeletion(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user("test_email_user", self.email, self.password)
        self.client.login(username=self.user.username)

    def test_account_deletion_correct_password(self):
        self.client.post(reverse("delete_account"), data={"auth-password": self.password})
        user = User.objects.get(username=self.user.username)
        self.assertFalse(user.is_active)

    def test_account_deletion_incorrect_password(self):
        self.client.post(reverse("delete_account"), data={"auth-password": "now-the-password"})
        user = User.objects.get(username=self.user.username)
        self.assertTrue(user.is_active)
...
class TestPublicMessage(TestCase):
    ...
    def contest_controller(self):
        self.model.objects.all().delete()
        contest = Contest.objects.get()
        contest.controller_name = self.controller_name
        contest.save()
        viewname_kwargs = getattr(self, "viewname_kwargs", {"contest_id": contest.id})
